...
object CucumberPlugin extends Plugin with Integration {
  ...
  private val projectVersion = "0.9.1"
  ...
  val cucumberSettingsWithTestPhaseIntegration = cucumberSettings ++ Seq(
    testFrameworks += new TestFramework("rocks.bdd.sbt.cucumber.CucumberFramework")
  )

  val cucumberSettingsWithIntegrationTestPhaseIntegration = cucumberSettings ++ Seq(
    testFrameworks in IntegrationTest += new TestFramework("rocks.bdd.sbt.cucumber.CucumberFramework"),
    libraryDependencies += "rocks.bdd" %% "sbt-cucumber-integration" % projectVersion % "it"
  )
}
